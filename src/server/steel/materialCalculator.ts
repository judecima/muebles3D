...
export function calculateSteelMaterials(config: SteelHouseConfig): MaterialEstimate {
  ...
  config.walls.forEach(wall => {
    ...
    wall.openings.forEach(op => {
      ...
      if (fusion === 'none') {
        ...
        pgc100_090 += numKings * 2 * studHeight;

      } else {

        pgc100_090 += numKings * studHeight;
  ...
  pushIfPositive({
    ...
    category: 'perfileria',
    unit: 'un',
    quantity: Math.ceil((pgc100_090 / BAR_LEN) * WASTE_STEEL),
    description: 'Montantes estructurales'
  });

  pushIfPositive({
    name: 'Perfiles PGC 100x1.25mm (6m)',
    category: 'perfileria',
    unit: 'un',
    quantity: Math.ceil((pgc100_125 / BAR_LEN) * WASTE_STEEL),
    description: 'Cordones de vigas reticuladas'
  });

  pushIfPositive({
    name: 'Perfiles PGC 100x1.60mm (6m)',
    category: 'perfileria',
    unit: 'un',
    quantity: Math.ceil((pgc100_160 / BAR_LEN) * WASTE_STEEL),
    description: 'Cordones reforzados de truss'
  });

  pushIfPositive({
    name: 'Perfiles PGC 100x2.00mm (6m)',
    category: 'perfileria',
    unit: 'un',
    quantity: Math.ceil((pgc100_200 / BAR_LEN) * WASTE_STEEL),
    description: 'Cordones de truss de grandes luces'
  });
  ...
  pushIfPositive({
    ...
    description: 'Soleras y bloqueos'
  });
  ...
  pushIfPositive({
    ...
    description: 'Montantes interiores'
  });
  ...
  pushIfPositive({
    ...
    description: 'Soleras interiores'
  });
  ...
  pushIfPositive({
    ...
    description: 'Diafragma estructural'
  });
  ...
  pushIfPositive({
    ...
    description: 'Barrera de agua y viento'
  });
  ...
  return {
    ...
    totalSteelWeightKg: Math.round(
      (pgc100_090 + pgc100_125 + pgc100_160 + pgc100_200 + pgu100Len) * 1.25 / 1000 +
      (pgc70Len + pgu70Len) * 0.6 / 1000
    )
  };
