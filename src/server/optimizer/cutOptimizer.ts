...
/**
 * JADSI Industrial Engine v27.1 - Sequential Independent Master
 * 
 ...
 * y decide su propia estrategia de guillotina (Eje X vs Eje Y).
 */
export function runOptimization(
  ...
  while (pool.some(p => !p.placed)) {
    ...
    for (let iter = 0; iter < iterationsPerPanel; iter++) {
      ...
      // Probar ambos sentidos de guillotina para este panel específico
  ...
  return {
    ...
    summary: `JADSI v27.1: ${finalPanels.length} paneles optimizados secuencialmente. Eficiencia Global: ${((totalUsedArea / totalAvailArea) * 100).toFixed(1)}%.`,
    kerf,
    trim,
    selectedThickness
  };
