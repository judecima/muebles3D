...
export function closetEngine(dim: FurnitureDimensions): FurnitureModel {
  ...
  const parts: Part[] = [
    ...
    { id: 'div-H', name: 'Divisor Horizontal', width: innerW, height: T, depth: D, x: W/2, y: drSectH, z: 0, type: 'static', cutLargo: innerW, cutAncho: Math.round(D), cutEspesor: T, grainDirection: 'horizontal' },
    { id: 'fondo', name: 'Fondo MDF 3mm', width: W - 2, height: H - 2, depth: 3, x: W/2, y: H/2, z: -D/2 - 1.5, type: 'static', cutLargo: Math.round(H - 2), cutAncho: Math.round(W - 2), cutEspesor: 3, grainDirection: 'libre' },
  ];
  ...
  const doorH = Math.round(doorSectH - 5);
  ...
  const gap = 3;
  const drFh = Math.round((drSectH - T - 10) / 2);
  const railCl = 13;
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const boxH = Math.round(drFh * 0.7);
    const boxInnerW = Math.round(drBoxW - 2 * T);

    parts.push({ id: `${prefix}-f`, groupId: prefix, name: `Frente Cajón`, width: W - 4, height: drFh - 2, depth: T, x: W/2, y: pY, z: D/2 + T/2, type: 'drawer', cutLargo: Math.round(drFh - 2), cutAncho: Math.round(W - 4), cutEspesor: T, grainDirection: 'horizontal' });
    ...
    parts.push({ id: `${prefix}-box-bottom`, groupId: prefix, name: `Piso Cajón 3mm`, width: drBoxW, height: 3, depth: drD, x: W/2, y: pY - boxH/2 + 1.5, z: D/2 - drD/2, type: 'drawer', cutLargo: drD, cutAncho: Math.round(drBoxW), cutEspesor: 3, grainDirection: 'libre' });
  ...
  return { parts, summary: 'Placard JADSI Industrial.', hasDoors: true, hasDrawers: true };
}
