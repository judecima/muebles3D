      ...
      <CardHeader className="bg-slate-900 text-white py-4 sticky top-0 z-10 shadow-sm">
        <CardTitle className="text-lg font-bold flex flex-col gap-0.5">
          <div className="flex items-center gap-2">
            ...
            <span>ArquiMax Structural v7.0</span>
          </div>
          <span className="text-[10px] opacity-70 font-normal uppercase tracking-widest">Motor de Cargas Industrial</span>
        </CardTitle>
      </CardHeader>
        ...
        <Accordion type="multiple" defaultValue={['global', 'layers', 'walls']} className="w-full">
          ...
          <AccordionItem value="global" className="border-b px-4">
            ...
            <AccordionContent className="space-y-4 pb-4">
              ...
              <p className="text-[9px] text-slate-400 italic">El ancho total define la carga tributaria sobre los muros longitudinales.</p>
            </AccordionContent>
          </AccordionItem>
            ...
            <AccordionContent className="space-y-3 pb-4">
              ...
              <div className="grid grid-cols-1 gap-2 bg-slate-50 p-3 rounded-xl border border-slate-100">
                {[
                  ...
                  { id: 'blocking', label: 'Blocking (Verde)', key: 'horizontalBlocking', color: 'text-green-600' },
                  { id: 'bracing', label: 'San Andrés (Ámbar)', key: 'bracing', color: 'text-amber-600' },
                  { id: 'junctions', label: 'Refuerzos (Azul/Rojo)', key: 'reinforcements', color: 'text-blue-600' }
                ].map(layer => (
          ...
          <AccordionItem value="walls" className="border-b px-4">
            <AccordionTrigger className="hover:no-underline py-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black uppercase tracking-tighter">Configuración de Muros</span>
              </div>
            </AccordionTrigger>
                ...
                {config.walls.map((wall, idx) => (
                  <div key={wall.id} className="border rounded-xl p-3 bg-slate-50/50 space-y-3 group">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black uppercase text-slate-700">Muro #{idx + 1} ({Math.round(wall.length)}mm)</span>
                      <Button size="icon" variant="ghost" className="h-6 w-6 text-red-400 opacity-0 group-hover:opacity-100" onClick={() => removeWall(wall.id)}>
