...
/**
 * Motor para Escritorio Red Arquimax v15.1
 * Estructura sándwich con gap de cajones de 3mm.
 */
export function deskEngine(dim: FurnitureDimensions): FurnitureModel {
  ...
  const parts: Part[] = [
    ...
    { id: 'desk-leg-L', name: 'Lateral Izquierdo Escritorio', width: T, height: H - 2 * T, depth: D, x: T / 2, y: H / 2, z: 0, type: 'static', cutLargo: H - 2 * T, cutAncho: D, cutEspesor: T, grainDirection: 'vertical' },
  ...
  const drawerD = D - 40;
  const drawerGap = 3; // Gap estandarizado 3mm
  const frontH = 190;
  const startY = cabinetH - 10; 

  for (let i = 0; i < 2; i++) {
    const posY = startY - (frontH / 2) - (i * (frontH + drawerGap));
  ...
  return { parts, summary: 'Escritorio v15.1: Estructura sándwich, gap 3mm.', hasDoors: false, hasDrawers: true };
}
