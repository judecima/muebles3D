...
export function closetEngine(dim: FurnitureDimensions): FurnitureModel {
  ...
  const parts: Part[] = [
    ...
    { id: 'base-inf', name: 'Base Inferior', width: innerW, height: T, depth: D, x: W/2, y: T/2, z: 0, type: 'static', cutLargo: innerW, cutAncho: D, cutEspesor: T, grainDirection: 'libre' },
    { id: 'tapa-sup', name: 'Tapa Superior', width: innerW, height: T, depth: D, x: W/2, y: H - T/2, z: 0, type: 'static', cutLargo: innerW, cutAncho: D, cutEspesor: T, grainDirection: 'libre' },
    { id: 'divisor-hor', name: 'Estante Divisor', width: innerW, height: T, depth: D, x: W/2, y: drawerSectionH, z: 0, type: 'static', cutLargo: innerW, cutAncho: D, cutEspesor: T, grainDirection: 'libre' },
  ];
  ...
  for (let i = 0; i < 2; i++) {
    ...
    parts.push({ id: `${prefix}-front`, name: `Frente Cajón ${i+1}`, width: W - 4, height: drawerFrontH, depth: T, x: W/2, y: posY, z: D/2 + T/2, type: 'drawer', cutLargo: drawerFrontH, cutAncho: W - 4, cutEspesor: T, grainDirection: 'libre' });
  ...
  return { parts, summary: 'Placard Red Arquimax modular.', hasDoors: true, hasDrawers: true };
}
