...
/**
 * Motor para Bajo Mesada Cajonera Paramétrico Red Arquimax v15.2
 * Estructura Sándwich + Cajones de 6 piezas sincronizadas + Rieles 13/13.
 */
export function kitchenDrawerEngine(dim: FurnitureDimensions): FurnitureModel {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    // 1. Frente Estético
    parts.push({ id: `${prefix}-front`, groupId: prefix, name: `Frente Cajón`, width: W - 4, height: frontH - 2, depth: T, x: W/2, y: posY, z: D/2 + T/2, type: 'drawer', cutLargo: frontH - 2, cutAncho: W - 4, cutEspesor: T, grainDirection: 'horizontal' });
    ...
    parts.push({ id: `${prefix}-box-bottom`, groupId: prefix, name: `Piso Cajón 3mm`, width: drawerBoxW, height: 3, depth: drD, x: W/2, y: posY - boxH/2 + 1.5, z: D/2 - drD/2, type: 'drawer', cutLargo: drD, cutAncho: drawerBoxW, cutEspesor: 3, grainDirection: 'libre' });
