...
export function kitchenCooktopEngine(dim: FurnitureDimensions): FurnitureModel {
  ...
  if (hasShelf) {
    parts.push({ id: 'shelf', name: 'Estante Interno', width: innerW - 2, height: T, depth: D * 0.9, x: W/2, y: doorY, z: 0, type: 'static', cutLargo: Math.round(innerW - 2), cutAncho: Math.round(D * 0.9), cutEspesor: T, grainDirection: 'horizontal' });
  }
