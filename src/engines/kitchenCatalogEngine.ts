...
export function kitchenCatalogEngine(type: FurnitureType, dim: FurnitureDimensions): FurnitureModel {
  ...
  // Configuración de apertura para 3 puertas: L - R - R
  // Puerta 1: Izq, Puerta 2: Der, Puerta 3: Der
  ...
  const createDrawer = (prefix: string, x: number, y: number, sectW: number, frontH: number, drawerDepth: number) => {
    ...
    // Estructura de Caja
    parts.push({ id: `${prefix}-box-F`, groupId: prefix, name: `Frente Estructura Cajón`, width: drawerBoxW - 2*T, height: boxH, depth: T, x: x, y: y, z: D/2 - T/2, type: 'drawer', cutLargo: drawerBoxH - 2*T, cutAncho: boxH, cutEspesor: T, grainDirection: 'libre' });
    parts.push({ id: `${prefix}-box-B`, groupId: prefix, name: `Trasera Estructura Cajón`, width: drawerBoxW - 2*T, height: boxH, depth: T, x: x, y: y, z: D/2 - drawerDepth + T/2, type: 'drawer', cutLargo: drawerBoxH - 2*T, cutAncho: boxH, cutEspesor: T, grainDirection: 'libre' });
  ...
  switch (type) {
    ...
    case 'cabinet_base_140_3p3c': {
      ...
      // Puertas con lógica L-R-R
      if (isLarge) {
        // 3 Puertas: 1 Izq, 2 y 3 Der. Necesita divisor extra.
    ...
    case 'cabinet_wall_3p': {
      ...
      // Puerta 2 (Centro, abre Der, pivota en div2)
      ...
      // Puerta 3 (Der, abre Der, pivota en lat-R)
    ...
    case 'cabinet_pantry_60_2p': {
      ...
      // Estantes estructurales fijos (no botón)
      for (let i = 1; i <= 4; i++) {
    ...
    case 'cabinet_microwave_60': {
      ...
      const upperSectH = H - lowerSectH - 500; // 500mm para microondas
    ...
    default: {
      // Si es un tipo no reconocido, retornar vacío para evitar errores
      break;
    }
  }
