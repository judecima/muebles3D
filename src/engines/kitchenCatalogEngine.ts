 ...
 * - Estructura Sándwich (Tapas W).
 * - Cajones de 6 piezas sincronizadas (groupId).
 * - Rieles 13mm/13mm simétricos.
 * - Huelgo: 2mm laterales, 3mm superior, 3mm entre cajones.
...
export function kitchenCatalogEngine(type: FurnitureType, dim: FurnitureDimensions): FurnitureModel {
  ...
  const addHinges = (doorId: string, dH: number, dW: number, pivotX: number, dY: number) => {
    const count = getHingeCount(dH, dW);
  ...
  switch (type) {
    ...
    case 'cabinet_base_120_2p3c': {
      ...
      parts.push({ id: 'div-dr', name: 'Divisor Cajonera', width: T, height: finalSideH - 20, depth: D * 0.9, x: divX - T/2, y: lateralY, z: 0, type: 'static', cutLargo: finalSideH - 20, cutAncho: D * 0.9, cutEspesor: T, grainDirection: 'vertical' });
      const drH = (H - T - 10) / 3;
    ...
    case 'cabinet_wall_140_3p': {
      ...
      const doorH = H - topGap;
      const doorY = H / 2;
      parts.push({ id: 'div', name: 'Divisor Estructural', width: T, height: finalSideH, depth: D * 0.9, x: divX, y: H/2, z: 0, type: 'static', cutLargo: finalSideH, cutAncho: D * 0.9, cutEspesor: T, grainDirection: 'vertical' });
      parts.push({ id: 'd-1', name: 'Puerta 1', width: doorW, height: doorH, depth: T, x: sideGap + doorW/2, y: doorY, z: D/2 + T/2, type: 'door-left', pivot: { x: 0, y: doorY, z: D/2 }, cutLargo: doorH, cutAncho: doorW, cutEspesor: T, grainDirection: 'vertical' });
      addHinges('d-1', doorH, doorW, 0, doorY);
      parts.push({ id: 'd-2', name: 'Puerta 2', width: doorW, height: doorH, depth: T, x: W/2, y: doorY, z: D/2 + T/2, type: 'door-right', pivot: { x: divX, y: doorY, z: D/2 }, cutLargo: doorH, cutAncho: doorW, cutEspesor: T, grainDirection: 'vertical' });
      addHinges('d-2', doorH, doorW, divX, doorY);
      parts.push({ id: 'd-3', name: 'Puerta 3', width: doorW, height: doorH, depth: T, x: W - sideGap - doorW/2, y: doorY, z: D/2 + T/2, type: 'door-right', pivot: { x: W, y: doorY, z: D/2 }, cutLargo: doorH, cutAncho: doorW, cutEspesor: T, grainDirection: 'vertical' });
  ...
  return { parts, summary: `Catálogo v15.2 Industrial. Estructura sándwich y cajones sincronizados.`, hasDoors, hasDrawers };
}