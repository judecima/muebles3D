...
/**
 * DATASET MESOPOTAMIA (23 PIEZAS EXACTAS XML LEPTON)
 * Utilizado para validar la eficiencia global industrial.
 */
...
export default function TestOptimizerPage() {
  ...
  const handleOptimize = () => {
    setLoading(true);
    // Simular tiempo de cálculo industrial determinístico
    setTimeout(() => {
      try {
      ...
      <div className="max-w-[1600px] mx-auto space-y-6 pb-20">
        ...
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between bg-white p-6 rounded-3xl shadow-sm border border-slate-100 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">ArquiMax Industrial Engine v11.5</h1>
              <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
                <Scissors className="w-3 h-3 text-primary" /> Optimización Global Jerárquica | Meta Lepton: 96.4%
              </p>
            </div>
          </div>
          <div className="flex gap-2 w-full md:w-auto">
            <Button size="lg" onClick={handleOptimize} disabled={loading} className="flex-1 md:flex-none font-black uppercase px-8 h-12 bg-slate-900 hover:bg-black text-white text-xs rounded-xl shadow-xl transition-all">
              ...
              {loading ? "Analizando Estrategias Globales..." : "Ejecutar Optimización Industrial"}
            </Button>
          </div>
        </header>
          ...
          <Card className="lg:col-span-1 shadow-sm border-none rounded-3xl overflow-hidden flex flex-col h-[800px]">
            <CardHeader className="border-b bg-slate-50/50 flex flex-row items-center justify-between p-4">
              <CardTitle className="text-[10px] font-black uppercase text-slate-500 tracking-widest flex items-center gap-2">
                <Layers className="w-4 h-4" /> Dataset Mesopotamia ({totalPartsCount})
              </CardTitle>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <Card className="shadow-sm border-none rounded-3xl p-6 bg-slate-900 text-white flex flex-col justify-center">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest">Diagnóstico ArquiMax v11.5</p>
                </div>
                <p className="text-[11px] text-slate-400 leading-relaxed font-medium">
                  {loading ? "Analizando layouts mediante motor jerárquico determinístico..." : 
                   result?.totalPanels === 1 ? "¡ÉXITO GLOBAL! Se ha consolidado todo el pedido en un solo panel de 2750x1830 mm mediante análisis de impacto global." : 
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                  <h2 className="text-sm font-black text-slate-800 uppercase tracking-tighter flex items-center gap-2">
                    <CheckCircle2 className={`w-4 h-4 ${result.totalEfficiency > 94 ? 'text-primary' : 'text-amber-500'}`} /> 
                    Esquema Técnico Industrial (DGP v11.5)
                  </h2>
                    ...
                    {result.optimizedLayout.map((panel, idx) => (
                      <div key={idx} className="bg-white p-10 rounded-xl shadow-2xl mb-12 border border-slate-100" 
                        ...
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <span className="text-[10px] font-black text-primary uppercase">{panel.efficiency.toFixed(2)}% APROVECHAMIENTO</span>
                        </div>
                          ...
                          <div className="absolute bg-white" style={{ 
                            ...
                            {panel.parts.map((p, pIdx) => (
                              <div key={pIdx} className="absolute border border-slate-900/40 transition-all hover:z-20 hover:brightness-95 group/piece cursor-help"
                                ...
                                {/* Medida Base (Ancho) - Línea Inferior */}
                                ...
                                {/* Medida Altura (Alto) - Línea Izquierda */}
                        ...
                        <div className="mt-4 flex items-center gap-2 text-slate-400">
                          ...
                          <span className="text-[9px] font-bold uppercase italic tracking-wider">Algoritmo ArquiMax v11.5: Análisis Sistémico Global Determinístico</span>
                        </div>
                      </div>
                    ))}
                  </div>
