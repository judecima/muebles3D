      ...
      <div className="max-w-[1600px] mx-auto space-y-6 pb-20">
        ...
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between bg-white p-6 rounded-3xl shadow-sm border border-slate-100 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">JADSI Industrial Engine v11.5</h1>
              <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
                <Scissors className="w-3 h-3 text-primary" /> Optimización Global Jerárquica
              </p>
            </div>
          </div>
          <div className="flex gap-2 w-full md:w-auto">
            <Button size="lg" onClick={handleOptimize} disabled={loading} className="flex-1 md:flex-none font-black uppercase px-8 h-12 bg-slate-900 hover:bg-black text-white text-xs rounded-xl shadow-xl transition-all">
              ...
              {loading ? "Calculando Estrategia JADSI..." : "Ejecutar Optimización"}
            </Button>
          </div>
        </header>
          ...
          <Card className="lg:col-span-1 shadow-sm border-none rounded-3xl overflow-hidden flex flex-col h-[800px]">
            <CardHeader className="border-b bg-slate-50/50 flex flex-row items-center justify-between p-4">
              <CardTitle className="text-[10px] font-black uppercase text-slate-500 tracking-widest flex items-center gap-2">
                <Layers className="w-4 h-4" /> Dataset de Prueba ({totalPartsCount})
              </CardTitle>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <Card className="shadow-sm border-none rounded-3xl p-6 bg-white flex flex-col justify-center">
                <div className="flex justify-between items-center mb-4">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Paneles</p>
                  <Layers className="w-4 h-4 text-primary" />
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between items-baseline">
                    <span className={`text-3xl font-black ${result?.totalPanels === 1 ? 'text-primary' : 'text-slate-800'}`}>
                      {result?.totalPanels || '-'}
                    </span>
              ...
              <Card className="shadow-sm border-none rounded-3xl p-6 bg-slate-900 text-white flex flex-col justify-center">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest">Diagnóstico JADSI v11.5</p>
                </div>
                <p className="text-[11px] text-slate-400 leading-relaxed font-medium">
                  {loading ? "Consultando motores industriales en el servidor..." : 
                   result ? "Optimización ejecutada exitosamente mediante algoritmos protegidos en el backend." : 
                   "Presiona ejecutar para validar la densidad global."}
                </p>
              </Card>
            </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                  <h2 className="text-sm font-black text-slate-800 uppercase tracking-tighter flex items-center gap-2">
                    ...
                    Esquema Técnico (Server-Side JADSI)
                  </h2>
                    ...
                    {result.optimizedLayout.map((panel, idx) => (
                      <div key={idx} className="bg-white p-10 rounded-xl shadow-2xl mb-12 border border-slate-100" 
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <span className="bg-slate-900 text-white px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest">PANEL 2750x1830</span>
                          <span className="text-[10px] font-black text-primary uppercase">{panel.efficiency.toFixed(2)}% USO</span>
                        </div>
                            ...
                            {panel.parts.map((p, pIdx) => (
                              <div key={pIdx} className="absolute border border-slate-900/40 transition-all hover:z-20 hover:brightness-95 group/piece cursor-help"
                                   ...
                                   style={{
                                     ...
                                     backgroundColor: p.color || 'rgba(13, 110, 253, 0.2)'
                                   }}>
                                <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 text-[min(1.2vw,9px)] font-black text-slate-900 pointer-events-none">
                                  {p.width}
                                </span>
                                <span className="absolute left-0.5 top-1/2 -translate-y-1/2 -rotate-90 origin-center text-[min(1.2vw,9px)] font-black text-slate-900 whitespace-nowrap pointer-events-none">
                                  {p.height}
                                </span>
